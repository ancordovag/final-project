...
def format_time(result):
    """ Receive a number of seconds and returns the corresponding time in format Hour:Minutes:Seconds: miliseconds """
...
def trainIters(encoder, decoder, n_iters, print_every=100, learning_rate=0.01, attention=True, recurrent='GRU'):
    """
    @param encoder: encoder to be trained
    @param decoder: decoder to be trained
    @param n_iters: number of iterations
    @param print_every: to print the time and the loss
    @param learning_rate: learning rate for the optimizer
    @param attention: if the decoder has the attention alignment
    @param recurrent: type of recurrent network
    @return encoder: trained encoder
    @return decoder: trained decoder
    """
    print("Training...")
    start = time.time()
...
def evaluate(encoder, decoder, sentence, max_length=MAX_LENGTH, recurrent_type ='GRU'):
    """
    @param encoder: the encoder trained
    @param decoder: the decoder trained
    @param sentence: sentence on the language of origin to be translated
    @param max_length: maximum length of the sentence to generate
    @param recurrent_type: type of recurrent network
    @return decoded_words: list of translated words
    @return decoder_attentions[:di + 1]: attention weights if the decoder uses attention
    """
    with torch.no_grad():
...
def evaluateRandomly(encoder, decoder, n=100, recurrent_type = 'GRU', display=True):
    ...
    @param encoder: the encoder trained
    @param decoder: the decoder trained
    @param n: number of sentence pairs to evaluate
    @param recurrent_type: type of recurrent network
    @param display: print the translations
    @return destinations: list of original sentences of the target language
    @return inferences: list of sentences generated by the decoder
    @return lengths: list of the length of the original sentences of the target language
    """
    destinations = []
    lengths = []
    inferences = []
    ...
    return destinations, inferences, lengths

if __name__ == '__main__':
    parser = ArgumentParser()
    parser.add_argument("--model_name", type=str, default="AKGRU", help="Name of the model, to save or to load")
    parser.add_argument("--epochs", type=int, default=10000, help="Number of training evaluations")
